    ...
    <el-card>
      ...
      <div class="text item">
        ...
        <div class="bottom clearfix">
          ...
          <router-link tag="a" :to='{name:"权限管理",params: { clientName: appDetails.clientName,clientId: appDetails.clientId}}' class="edit routeTag" @click="storageHandle(appDetails)">编辑权限</router-link>
        </div>
      </div>
    </el-card>
    <!-- 权限录入 -->
                ...
                <el-table-column label="模块名称">
                  <template slot-scope="scope" v-if="scope.row.moduleName">
                    <el-tooltip :content="scope.row.moduleName" placement="top" effect="dark">
                      <span>{{scope.row.moduleName.length > 10 ? scope.row.moduleName.substr(0,9)+'...' : scope.row.moduleName}}</span>
                    </el-tooltip>
                  </template>
                </el-table-column>
...
export default {
  ...
  methods:{
      ...
      getModuleById(param).then(res=>{
        if(res.err_CODE === 0){
            res.data.map(item=>{
                ...
                item.label = item.moduleName.length > 8 ? (item.moduleName).substring(0,7)+"..." :item.moduleName;
                item.name = item.moduleName;
                item.level = 2;

                item.functionList.map(jtem =>{
                  jtem.id = jtem.functionId;
                  jtem.label = jtem.functionName.length > 8 ?  (jtem.functionName).substring(0,7)+"..." : jtem.functionName;
                  jtem.level = 3;
                  ...
                  if(jtem.apiUrlList.length >0){
                    ...
                    if(jtem.url.length > 8){
                      jtem.url = (jtem.url).substring(0,7)+"...";
                    }else{
                      jtem.url = jtem.url;
                    }
                  }else{
            ...
            let root = {
                ...
                label:this.appDetails.clientName.length>8 ? this.appDetails.clientName.substring(0,7)+"..." : this.appDetails.clientName,
        ...
        if(res.err_CODE === 0){
          ...
          this.functionData.map(item =>{
            ...
            if(item.apiUrlList.length > 0){
              ...
              url.length > 8 ? url.substring(0,7)+"..." : url;
              item.url = url;
            }else{
              item.url = "--";
            }
    ...
    storageHandle(param){
      /**
       * @description 本地存储clientId
       * @param {Number} clientId
       */
      let params = {clientId: param.clientId, clientName: param.clientName};
      window.localStorage.setItem("clientDetails", JSON.stringify(params));
    }
  }
}
</script>
